...
```bash
# Core engine — required for everything
...
# Option B: Self-hosted / free (built into @ingissa/navcore-core — no extra install)
# OSRMDirectionsProvider, ValhallaDirectionsProvider, OpenRouteServiceProvider

# Renderer (choose one)
npm install @ingissa/navcore-maplibre      # Web — MapLibre GL JS
npm install @ingissa/navcore-leaflet       # Web — Leaflet.js
npm install @ingissa/navcore-google-maps   # Web — Google Maps API
...
const engine = new NavCore({
  isDev: true,  // use in development — remove in production and add licenseKey
});
```
